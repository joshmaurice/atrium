...
export async function setupCommons({ registry, db, worldPath }) {
  ...
  if (!operatorUsername) {
    return degradedBoot(registry, db, worldPath, 'ATRIUM_COMMONS_OWNER is not set')
  }
  ...
  try {
    ...
    return degradedBoot(registry, db, worldPath, 'DB error looking up existing commons row')
  }
  ...
  if (existingRow && existingRow.visibility !== 'public') {
    ...
    return degradedBoot(registry, db, worldPath, 'Existing commons row is not public')
  }
  ...
  if (existingRow) {
    ...
    try {
      ...
      return degradedBoot(registry, db, worldPath, 'Failed to insert commons row')
    }
  }
...
async function degradedBoot(registry, db, worldPath, reason) {
  console.error(
    `[commons] DEGRADED READ-ONLY BOOT — ${reason}. The commons is readable via / by everyone but ALL mutation is denied.`
  )

  // Clean any stale 'default' host
  if (registry.hosts.has('default')) {
    const oldHost = registry.hosts.get('default')
    oldHost.close()
    registry.hosts.delete('default')
    console.log('[commons] Removed stale "default" host during degraded boot')
  }

  // Register the file world with read-only policy
  const host = await registry.registerWorld('default', worldPath, null, 'read-only')
  registry.setRootWorldId('default')

  const nodeCount = host.world.listNodeNames().length
  console.log(`[commons] Commons ready: mode="read-only", key="default", nodes=${nodeCount}`)
